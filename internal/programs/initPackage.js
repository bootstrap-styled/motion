...
function initPackage(args, cb) {
  ...
  async.auto({
    ...
    gitignore: (done) => {
      const output = path.join(__dirname, '../../.gitignore');
      const before = 'lib\n|dist\n'; // eslint-disable-line no-useless-escape
      const after = '';
      bash.sedReplace(output, before, after, output, done);
    },
    rollupPkg: (done) => {
      const pkg = require(path.join(__dirname, '../../package.json')); // eslint-disable-line global-require
      done(null, pkg);
    },
    ...
    createReadme: ['rollupPkg', 'createChangelog', (results, done) => {
      ...
      bash.sedReplace(input, before, after, output, () => {
        ...
        bash.sedReplace(output, beforeGit, afterGit, output, () => {
          ...
          bash.sedReplace(output, beforeHttp, afterHttp, output, () => {
            const beforeYear = '$YEAR';
            const afterYear = new Date().getFullYear();
            bash.sedReplace(output, beforeYear, afterYear, output, () => {
              const beforeVersion = '$ROLLUP_VERSION';
              const afterVersion = results.rollupPkg.version;
              bash.sedReplace(output, beforeVersion, afterVersion, output, done);
            });
          });
        });
      });
